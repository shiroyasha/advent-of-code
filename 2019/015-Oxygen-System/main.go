...
func (r *Robot) move(direction int) int {
	r.process.AddInput(direction)
	r.process.RunTilInterupt()

	return r.process.NextOutput()
}
...
func (r *Robot) check(direction, distance int, visits map[Pos]int) int {
	dis := distance

	r.pos.Add(movement[direction])

	if d, ok := visits[r.pos]; ok {
		dis = d
	} else {
		switch r.move(direction) {
		case 0:
			dis = 10000000000
		case 1:
			visits[r.pos] = true

			dis = r.findOxygen(distance+1, visits)

			visits[r.pos] = false
			r.move(oposites[direction])
	...
	r.pos.Substract(movement[direction]


	panic("AAA")
}

func (r *Robot) findOxygen(distance int, visits map[Pos]bool) int {
	dis1 := r.check(1, distance, visits)
	dis2 := r.check(2, distance, visits)
	dis3 := r.check(3, distance, visits)
	dis4 := r.check(4, distance, visits)
