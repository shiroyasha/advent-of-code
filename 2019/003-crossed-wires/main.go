...
type Plane [20000][20000]bool
type VisitPlane [20000][20000]bool

var (
	PlaneCenter = Point{X: 10000, Y: 10000}
)

func (w *Wire) MapPlane(intersections []Point) *Plane {
	plane := &Plane{}
	...
	for _, s := range w.Segments() {
		for _, p := range s.Points() {
			plane[PlaneCenter.Y+p.Y][PlaneCenter.X+p.X] = true
		}
	}
...
func TravelSteps(plane *Plane, a Point, b Point, visited *VisitPlane) int {
	visited[PlaneCenter.Y+a.Y][PlaneCenter.X+a.X] = true
	...
	if plane[PlaneCenter.Y+a.Y][PlaneCenter.X+a.X+1] && !visited[PlaneCenter.Y+a.Y][PlaneCenter.X+a.X+1] {
		cost := 1 + TravelSteps(plane, Point{X: a.X + 1, Y: a.Y}, b, visited)
	...
	if plane[PlaneCenter.Y+a.Y+1][PlaneCenter.X+a.X] && !visited[PlaneCenter.Y+a.Y+1][PlaneCenter.X+a.X] {
		cost := 1 + TravelSteps(plane, Point{X: a.X, Y: a.Y + 1}, b, visited)
...
func ResetVisits(plane *VisitPlane) {
	for i := 0; i < 20000; i++ {
		for j := 0; j < 20000; j++ {
			plane[i][j] = false
		}
...
func FindMinStepsToReach(plane1 *Plane, plane2 *Plane, intersections []Point) int {
	min := math.MaxInt32
	...
	for _, i := range intersections {
		...
		ResetVisits(visits)
		c1 := TravelSteps(plane1, Point{X: 0, Y: 0}, i, visits)

		ResetVisits(visits)
		c2 := TravelSteps(plane1, Point{X: 0, Y: 0}, i, visits)
...
func main() {
	...
	plane1 := wire1.MapPlane(intersections)
	plane2 := wire2.MapPlane(intersections)

	steps := FindMinStepsToReach(plane1, plane2, intersections)
